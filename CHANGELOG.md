...
- URL de clonado del README es la pública `https://github.com/Juancit015/MultiBot.git`, idéntica al remote actual de `origin` (HTTPS).
...
- El default `BOT_API_BASE_URL=https://multi-api-production.up.railway.app/bot` (`bot/config.py`) producía peticiones `.../bot<TOKEN>/getMe`, ruta que el gateway `multi-api-production.up.railway.app` no enruta (404 incluso con token válido; el gateway espera `<TOKEN>/<método>` sin `/bot`). Se documentó primero sin tocar código; tras aprobación del mantenedor se corrigió el default en `bot/config.py` → `https://api.telegram.org/bot` (oficial, verificado).
...
- Guard de regresión `no_live_requests` ampliado en `tests/conftest.py`: bloquea además `tikwm.requests.post` y `net.requests.get/head`; fixture autouse `no_real_resolve_short_url` mockea `resolve_short_url` en `bot/handlers/media.py`. La suite (27 tests) ya no depende de la red: completa en ~5s en lugar de ~15s.
- Servicio nuevo `bot/services/instagram.py` con `carousel_sidecar(shortcode)`; `bot/handlers/instagram.py` delega en él (se elimina el import muerto `IG_SESSION` y se aísla la IO del handler).
- `sc_search(query, folder)` extraída en `bot/services/ytdlp.py`; `bot/handlers/soundcloud.py` deja de usar `yt_dlp` directamente y los mocks de `tests/test_soundcloud.py` (T13) apuntan a `bot.services.ytdlp.yt_dlp.YoutubeDL` + `make_opts`.
- `.dockerignore` excluye `tests/`, `requirements-dev.txt` y `pytest.ini` (la imagen conserva solo lo de producción).
- Deuda técnica no bloqueante documentada: H4 (`httpx` 0.28.1 transitiva vía instaloader, sin uso directo) y H6 (`imgs[:10]` en `bot/handlers/net.py` recorta carruseles sin chunking).

### Eliminado

- `multibot.py`: `import os` muerto.
- `cookies_yt.txt` (121 KB) huérfano, no referenciado desde el código, eliminado del repo (patrón `cookies*.txt` ya ignorado).